...
## inputs:
#     path_cellranger_output <- paste(sep = "\\", path_raw_data_file, "hub_11_outs\\multi\\count\\raw_feature_bc_matrix.h5")
#     path_cellbender_output <-  paste(sep = "\\", path_raw_data_file,"cellbender\\hub_11\\hub_11_cellbender_corrected_filtered_seurat.h5")
#    convert_vector <- c("hIO_BIHi001-B"="CMO305",
#     "hIO_BIHi005-A"="CMO306",
#     "hIO_BIHi250-A"="CMO307",
#     "hIO_UCSFi001-A"="CMO308",
#     "LOA_BIHi005-A"="CMO309",
#     "LOA_BIHi250-A"="CMO310",
#     "LOA_UCSFi001-A"="CMO311")
#
#     tag_calls_summary <- read_csv(paste(sep = "\\", path_raw_data_file,  "hub_11_outs/multi/multiplexing_analysis/tag_calls_summary.csv"))
#     tag_calls_per_cell <- read_csv(paste(sep = "\\", path_raw_data_file, "hub_11_outs/multi/multiplexing_analysis/tag_calls_per_cell.csv"))
#     assignment_confidence_table <- read_csv(paste(sep = "\\",path_raw_data_file,  "hub_11_outs/multi/multiplexing_analysis/assignment_confidence_table.csv"))
...
#                         "_max.cells.per.ident_",max_cells_,
#                         ".csv"))
# write_rds(seurat_obj, "intermediate_data/QC_dataset_setup_hub_11.rds")
...
path_cellranger_output <- paste(here(path_raw_data_file, "cellranger","hub_11_outs","multi","count" ,"raw_feature_bc_matrix.h5"))
path_cellbender_output <- paste(here(path_raw_data_file, "cellbender","hub_11"  ,"hub_11_cellbender_corrected_filtered_seurat.h5"))

## set hashtag conversion
convert_vector <- c("CMO305"="hIO_BIHi001-B",
                    "CMO306"="hIO_BIHi005-A",
                    "CMO307"="hIO_BIHi250-A",
                    "CMO308"="hIO_UCSFi001-A",
                    "CMO309"="LOA_BIHi005-A",
                    "CMO310"="LOA_BIHi250-A",
                    "CMO311"="LOA_UCSFi001-A")


tag_calls_summary <- read_csv(paste(sep = "\\", path_raw_data_file, "cellranger\\hub_11_outs\\multi\\multiplexing_analysis/tag_calls_summary.csv"))
tag_calls_per_cell <- read_csv(paste(sep = "\\", path_raw_data_file, "cellranger\\hub_11_outs\\multi\\multiplexing_analysis/tag_calls_per_cell.csv"))
assignment_confidence_table <- read_csv(paste(sep = "\\", path_raw_data_file, "cellranger\\hub_11_outs\\multi\\multiplexing_analysis\\assignment_confidence_table.csv"))
...
if (calc_bp_cells) {
  ...
  write_matrix_dir(
    mat = dat,
    dir = paste0("BPcell_matrices/", dataset_name)
  )
  ...
  write_matrix_dir(dat_cellbender$`Gene Expression`, dir = paste0("BPcell_matrices/", dataset_name, "/cellbender"))
}
#############################################################################################
...
dat <- open_matrix_dir(dir = paste0("BPcell_matrices/", dataset_name))
...
for (assay_it in Assays(seurat_obj)) {
  ...
  seurat_obj <- seurat_obj |> FindClusters(
    resolution = 0.8,
    verbose = TRUE,
    cluster.name = paste0(assay_it, basic_cluster_name), 
...
corrected <- H5Fopen(here(path_raw_data_file,"cellbender","hub_11","hub_11_cellbender_corrected.h5"))
