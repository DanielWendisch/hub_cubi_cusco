...
step_no <- "initial_variables"

# variables shared in  whole workflow, meaning setup and filtering steps
dataset_name <- "hub_01"
...
documentation_tbl <- tibble(dataset_name=dataset_name, 
       step_no=step_no,
       clustering=leiden_or_louvain,
       )
```
#------
...
add_vireo <- FALSE # wether to add SNP based demultiplexing conducted with vireo to dataset
calc_bp_cells <- TRUE # FALSE if previous computation shiuld be used
...
generate_plot_grobs <- function(step, dataset = dataset_name) { 
  # creates plots specified in R script prefiltering plots
  # writes grob list and ggextra grob list to disk 
  ...
  plotified_ggextra_plot_list_cellranger_cells |> write_rds(path_ggextra_filtered)
...
output_dir <- path_output_plots_root
dir.create(output_dir, showWarnings = FALSE)

# Save each grob using its name
for (name in names(plot_list_grob)) {
  g <- plot_list_grob[[name]]
  
  # Define filename
  file_path <- file.path(output_dir, paste0(name, ".pdf"))
  
  # Save as PDF (you can switch to PNG if needed)
png(file_path <- file.path(output_dir, paste0(name, ".png")), width = 700, height = 500)
grid.draw(g)
dev.off()
}
...
seurat_obj |> DimPlot(group.by = "RNA_clusters_louvain_res._0.8", label = TRUE)
```
...
seurat_obj_filtered |> DimPlot(group.by = "RNA_clusters_louvain_res._0.8", label = TRUE)
deleted_clusters <- c(10,12,15,14)
seurat_obj_filtered |>
  filter((.data[["RNA_clusters_louvain_res._0.8"]] %in% deleted_clusters)) |>
  DimPlot(group.by = "RNA_clusters_louvain_res._0.8", label = TRUE)

```

```{r}

seurat_obj_filtered <- seurat_obj_filtered |> filter(!(.data[["RNA_clusters_louvain_res._0.8"]] %in% deleted_clusters) )
