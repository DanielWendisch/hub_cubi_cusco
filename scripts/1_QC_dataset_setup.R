...
# if (is_snakemake) print("executing snakmake options")
# if (is_snakemake) print("executing snakmake options")
# if (!is_snakemake) {
# dataset_name <- "hub_02"
...
# tag_calls_per_cell <- read_csv(paste(sep = "\\", path_raw_data_file, "hub_02_outs/multi/multiplexing_analysis/tag_calls_per_cell.csv"))
#
# assignment_confidence_table <- read_csv(here(path_raw_data_file,"cellranger", "hub_02_outs","multi","multiplexing_analysis","assignment_confidence_table.csv"))}

########################## SNAKEMAKE
if (!is_snakemake) print("NOT executing snakemake options")

if (is_snakemake) {
  print("executing snakemake options")

  dataset_name <- snakemake@params[["dataset"]]

  path_cellranger_output <- snakemake@input[["cellranger_output"]]
  path_cellbender_output <- snakemake@input[["cellbender_output"]]
  ...
  tag_calls_summary <- snakemake@input[["tags_summary"]] |> read_csv()
  tag_calls_per_cell <- snakemake@input[["tags_per_cell"]] |> read_csv()
  assignment_confidence_table <- snakemake@input[["confidence_table"]]
}
###################################
...
print("creating BPCells matrices")
...
if (calc_bp_cells) {
  ...
  write_matrix_dir(
    mat = dat,
    dir = here("BPcell_matrices", dataset_name)
  )

  write_matrix_dir(
    mat = cmo_dat,
    dir = here("BPcell_matrices", "cmo_", dataset_name)
  )
  ...
  ### ----------------------------------------------------------------------------------------------
  print("1")
  print(path_cellbender_output)
  dat_cellbender <- Read10X_h5(filename = path_cellbender_output, unique.features = TRUE)

  print("2")
  setdiff(
    rownames(dat_cellbender)[!(grepl("ENSG", rownames(dat_cellbender)))],
    rownames(dat)
  ) |> length()

  print("3")
  bpcells_cellbender_path <- here("BPcell_matrices", dataset_name, "cellbender")
  print(bpcells_cellbender_path)
  write_matrix_dir(dat_cellbender, dir = bpcells_cellbender_path)
}
#############################################################################################
...
dat_cellbender <- open_matrix_dir(dir = bpcells_cellbender_path)
...
for (assay_it in Assays(seurat_obj)) {
  ...
  seurat_obj <- seurat_obj |>
    RunUMAP(
      dims = 1:30, reduction = paste0("pca.", assay_it),
      assay = assay_it,
...
write_csv(seurat_obj.markers, file = here(
  "output",
  paste0(
    "QCmarkers_min.pct_", min_pct,
    "_logfc.threshold_", logfc_threshold,
