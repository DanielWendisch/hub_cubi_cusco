...
path_output_file <- here("output", "BayesPrism_5_clustsers")
...
```{r}
cell.state.labels <- obj |> pull(harmony_clusters_res0.1) |> as.character()

obj |> ggplot(aes(cell_line_cellranger, fill = harmony_clusters_res0.1, colour = m_e_class)) + geom_bar(linewidth=1)+
  scale_color_manual(values=c("black", "red"))+
  theme_bw()

ggsave(paste0(path_output_file, "/hub_01_barplot_numbers_harmony_clusters_res0.1.png"))
...
sc_counts_harmony_clusters_res0.1 <- obj |>
  group_by(cell_line_cellranger, harmony_clusters_res0.1) |>
  summarise(n=n())|>
  mutate(fraction=n/sum(n)) |>
  separate(cell_line_cellranger, sep = "_", c("organoid_type",
                                   "cell_line")) |> 
  rename(fraction_bulk="fraction",
        cluster="harmony_clusters_res0.1" ) |> 
  select(cell_line, cluster,fraction_bulk )

sc_counts_me
sc_counts_harmony_clusters_res0.1
```
...
dat <- proportions_cell_states |>
  ...
  left_join(sc_counts_harmony_clusters_res0.1)
dat
```
...
proportions_cell_states |>
  pivot_longer(starts_with("GEX"),
               ...
               values_to = "fraction") |> 
  left_join(select(hub_01_meta_data, cell_line,processing, sample)) |> 
  left_join(rename(sc_counts_harmony_clusters_res0.1,cell_line_bulk="cell_line"))|>
  ...
  ggtitle("KL bulk predicting sc proportions, 5 clusters")



ggsave(paste0(path_output_file, "/fraction_corr_plot_KL bulk predicting sc proportions 5 clusters.png")) 
```
```{r}
proportions_cell_states |>
  pivot_longer(starts_with("GEX"),
